...
struct FpToFpOpConversion : public ConvertOpToLLVMPattern<triton::FpToFpOp> {
  ...
  matchAndRewrite(triton::FpToFpOp op, OpAdaptor adaptor,
    ...
    auto convertOne = [&](Value in) -> FailureOr<Value> {
      // Fast path: standard floating-point cast without fp8.
      if (!srcIsFp8E5 && !dstIsFp8E5)
        return emitFloatCast(in, llvmDstElemTy);

      // Metal fp8 lowering currently supports e5m2/fnuz variants only.
      if (srcIsFp8E5 && !dstIsFp8E5) {
      ...
      // fp8 -> fp8 (same type) is a no-op at this stage because fp8 is
      // represented as i8 bits in LLVM lowering.
      if (srcIsFp8E5 && dstIsFp8E5) {
