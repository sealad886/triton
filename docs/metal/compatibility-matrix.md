...
- `PYTHONPATH=python .venv/bin/python scripts/metal_release_checks.py --profile hosted-ci --tag first-class-hosted-ci`
  - success (backend tests, `test_ir_types`, smoke, cross-backend numerics, AOT checks)

Hosted correctness gating is the canonical first-class support bar. The local
